 const CrmFileUploader = (function() {
     /**
      * 构造函数
      *
      * @param {any} options
     ...
     function generateUploadArea(options) {
         ...
         let html = `
            ...
            <div class="upload-area fll">
                ...
                <form id=${isUpperIE9() ? 'crm-upload-form' :'ie9-crm-upload-form'}
                class="clearfix" method="post" enctype="multipart/form-data"
                action="${options.uploadUrl}" ${!isUpperIE9() ? 'target="hidenIframe"' :''}>
                    <label class="upload-file"><input type="file" name="file" />${options.uploadHint || '添加附件'}</label>
                </form>
        `;

         if (!isUpperIE9()) {
             html += `<iframe name="hidenIframe" id="hidenIframe"
             style="display:none" src="" frameborder="“0”" scrolling="yes" height="0px" width="0px"></iframe>`;
         }
     ...
     function bindEventHandler() {
         ...
         $('#crm-upload-form input[type="file"]').change(function(e) {
             // 文件流数组
             let fileStream = $(this).prop('files')[0];
         ...
         $('#ie9-crm-upload-form #fileSelector').change(function(e) {
             let ie9FileName = parseFilePath(this); // ie9下的文件名
     ...
     function parseFilePath(inputElement) {
         let filePath = $(inputElement).val();
         let fileName = filePath.split('\\').pop(); // clean from C:\fakepath OR C:\fake_path
         return fileName;
     }

     /**
      * 获取上传文件的进度，如果上传成功了，在隐藏的iframe中的内容里会返回对应的附件id
      *
      * @param {any} ie9FileName
      * @returns
      */
     function getUploadProgress(ie9FileName) {
         // 获取隐藏的iframe中的内容
         let hidenIframe = $('#hidenIframe')[0];
         let hidenBody = $(hidenIframe.contentDocument).find('body')[0];
         let hidenResponse = $(hidenBody).html();
      ...
      * @param {any} uploadFileName 上传文件的名称
     ...
     function appendToAttachmentList(attachmentId, fileName) {
         let newFileItemStr = '<li><em id="attachment-id" style="display:none">' + attachmentId + '</em><i  class="downloadable-attchment">' + fileName + '</i>;<span></span></li>';
     ...
     function bindDeleteAttachmentHandler() {
         ...
         $(this.options.UPLOAD_WRAPPER_SELECTOR + ' .attachment-list  li span').click(function(e) {
             let attachmentId = parseInt($(this).parent().find('#attachment-id').html().trim()); // 找到对应的附件id
     ...
     function bindDownloadAttachmentHandler() {
         ...
         $(this.options.UPLOAD_WRAPPER_SELECTOR + ' .attachment-list  li .downloadable-attchment').click(function(e) {
             ...
             let attachmentId = $(this).parent().find('#attachment-id').html().trim(); // 附件id
